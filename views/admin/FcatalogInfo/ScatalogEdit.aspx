<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ScatalogEdit.aspx.cs" Inherits="admin_FcatalogInfo_ScatalogEdit" %>
...
<body  onload="scacominput();scatainit()">
       <form id="form1" runat="server">
    <!--标题-->
    <div class="alltitle">
        <div class="left"></div>
        <div class="center">目录编辑</div>
        <span>当前位置：课程信息管理>>二级目录>><label id="edit_tile" runat="server">添加目录</label></span>
        <div class="right"></div>
        <div class="titlebtn"></div>
    </div>
    <div class="edit">
        <div class="edit_top">
            <div><input type="text" id="txtsname"  autocomplete="off" name="txtsname" runat="server"/><span>目录名称：</span></div>
            <div><input type="text" id="txtcname" autocomplete="off" onchange="getfatalog()"   name="txtcname" runat="server"/><span>课程名称：</span></div>
            <div><select  id="rdlfname" name="rdlfname" runat="server"></select>
                <span>一级目录：</span>
            </div>
            <div>
        ...
        <div class="edit_submit"><div>
                <asp:Button ID="btnsutmit" UseSubmitBehavior="false"  runat="server" Text=" " CssClass="bgadd" onclick="btnsutmit_Click" OnClientClick="if(!scatalogedit()) {return false;}" />
