...
function showaddclasstype() {
    ...
    var submit = function (v, h, f) {
        if (v == true) {
            ...
            var success = admin_CourseInfo_CourseType.AddClassType(f.txtnewstype).value;  //-1,1,2
            if (success == 1) {
                top.$.jBox.tip('添加成功！', 'success', { timeout: 1000, closed: function () { location.reload(true) } });
                admin_CourseInfo_CourseType.UserLog("Add").value;
            } else if (success == 2) {
...
function showeditclasstype(id, txttype) {
    ...
    var submit = function (v, h, f) {
        if (v == true) {
            ...
            var success = admin_CourseInfo_CourseType.EditClassType(id, f.txtnewstype).value;
            //-1,1,2
            if (success == 1) {
                top.$.jBox.tip('修改成功！', 'success', { timeout: 1000, closed: function () { location.reload(true) } });
                admin_CourseInfo_CourseType.UserLog("Update").value;
            } else if (success == 2) {
...
function showdel() {
    ...
    var idstr = GetIdByArry();
    var submit = function (v, h, f) {
        if (v == 'ok') {
            top.$.jBox.tip("正在删除数据...", 'loading');         // 模拟2秒后完成操作   
            var success = admin_CourseInfo_CourseType.DelClassType(idstr).value;
            if (success > 0) {
                top.$.jBox.tip('删除成功！', 'success', { timeout: 1000, closed: function () { location.reload(true) } });
                admin_CourseInfo_CourseType.UserLog(function (red) {
                    if (red.value < 0) {
